...
parser.add_argument('--filtering_ckpt', type=str, default='best_model_epoch75.pt', help='Checkpoint to use for the confidence model')
...
test_dataset = PDBBind(transform=None, root='', protein_path_list=protein_path_list, ligand_descriptions=ligand_descriptions,
                       ...
                       fixed_knn_radius_graph = False if not hasattr(score_model_args, 'not_fixed_knn_radius_graph') else not score_model_args.not_fixed_knn_radius_graph,
                       ...
                       include_miscellaneous_atoms= False if not hasattr(score_model_args,'include_miscellaneous_atoms') else score_model_args.include_miscellaneous_atoms,
                              use_old_wrong_embedding_order = True if not hasattr(score_model_args,'use_old_wrong_embedding_order') else score_model_args.use_old_wrong_embedding_order)
...
if args.filtering_model_dir is not None:
    if not (filtering_args.use_original_model_cache or filtering_args.transfer_weights): # if the filtering model uses the same type of data as the original model then we do not need this dataset and can just use the complexes
        ...
        filtering_test_dataset = PDBBind(transform=None, root='', protein_path_list=protein_path_list,
                                         ligand_descriptions=ligand_descriptions,
                                         chain_cutoff=np.inf,
                                         receptor_radius=filtering_args.receptor_radius,
                                         cache_path=args.cache_path,
                                         remove_hs=filtering_args.remove_hs, max_lig_size=None,
                                         ...
                                         atom_radius=filtering_args.atom_radius,
                                         atom_max_neighbors=filtering_args.atom_max_neighbors,
                                         esm_embeddings_path=args.esm_embeddings_path if filtering_args.esm_embeddings_path is not None else None,
                                         require_ligand=True,
                                         num_workers=args.num_workers,
                                         fixed_knn_radius_graph=False if not hasattr(filtering_args, 'not_fixed_knn_radius_graph') else not filtering_args.not_fixed_knn_radius_graph,
                                         knn_only_graph=False if not hasattr(filtering_args, 'not_knn_only_graph') else not filtering_args.not_knn_only_graph,
                                         include_miscellaneous_atoms= False if not hasattr(filtering_args,'include_miscellaneous_atoms') else filtering_args.include_miscellaneous_atoms,
                                        use_old_wrong_embedding_order = True if not hasattr(filtering_args,'use_old_wrong_embedding_order') else filtering_args.use_old_wrong_embedding_order)
...
for idx, orig_complex_graph in tqdm(enumerate(test_loader)):
    ...
    try:
        ...
        data_list, confidence = sampling(data_list=data_list, model=model,
                                         ...
                                         ode=args.ode, visualization_list=visualization_list, confidence_model=filtering_model, filtering_data_list=filtering_data_list, filtering_model_args=filtering_model_args,

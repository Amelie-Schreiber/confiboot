...
parser.add_argument('--results_path', type=str, default='results/gnina_predictions_noHs2', help='Path to folder with trained model and hyperparameters')
parser.add_argument('--file_suffix', type=str, default='_baseline_ligand.sdf', help='Path to folder with trained model and hyperparameters')
...
for i, name in enumerate(tqdm(names)):
    ...
    if args.all_dirs_in_results:
        ...
        if not os.path.exists(os.path.join(args.results_path, name, f'{"" if args.no_id_in_filename else name}{args.file_suffix}')):
            print('skipping because path did not exists:', os.path.join(args.results_path, name, f'{"" if args.no_id_in_filename else name}{args.file_suffix}'))
            continue
        molecule_file = os.path.join(args.results_path, name, f'{"" if args.no_id_in_filename else name}{args.file_suffix}')

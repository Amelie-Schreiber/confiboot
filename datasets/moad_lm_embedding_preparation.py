...
for name in tqdm(names):
    ...
    if not os.path.exists(os.path.join(data_dir, f'{name}_protein_chain_removed.pdb')):
        print(f"We are skipping {name} because there was no {name}_protein_chain_removed.pdb")
        continue
    rec_path = os.path.join(data_dir, f'{name}_protein_chain_removed.pdb')
    try:

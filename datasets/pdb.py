...
from datasets.process_mols import new_extract_receptor_structure, get_lig_graph
...
class PDBSidechain(Dataset):
    def __init__(self, root, transform=None, cache_path='data/cache', split='train', limit_complexes=0,
                 ...
                 vandermers_min_contacts=5, remove_second_segment=False, merge_clusters=1):

        super(PDBSidechain, self).__init__(root, transform)
        assert remove_hs == True and all_atoms == False, "not implemented yet"
        self.root = root
        self.split = split
    ...
    def read_split(self):
        ...
        # df = df[df['CHAINID'].str.contains("l3")]  # TODO remove
    ...
    def load_chain(self, c):
        ...
        try:
            new_extract_receptor_structure(seq, coords, complex_graph=complex_graph, neighbor_cutoff=self.receptor_radius,

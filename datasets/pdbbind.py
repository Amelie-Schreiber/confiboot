...
from datasets.process_mols import read_molecule, get_rec_graph, \
    get_lig_graph_with_matching, extract_receptor_structure, parse_receptor, parse_cross_receptor, parse_pdb_from_path, \
    generate_conformer, moad_extract_receptor_structure
from utils.diffusion_utils import modify_conformer, get_inverse_schedule, set_time
from utils.utils import read_strings_from_txt, crop_beyond
from utils import so3, torus


class NoiseTransform(BaseTransform):
    def __init__(self, t_to_sigma, no_torsion, all_atom, alpha=1,beta=1, rot_alpha=1, rot_beta=1, tor_alpha=1,
                 tor_beta=1, separate_noise_schedule=False, asyncronous_noise_schedule=False,
    ...
    def get_time(self):
        ...
        elif self.separate_noise_schedule:
            t = None
    ...
    def apply_noise(self, data, t_tr, t_rot, t_tor, t, tr_update = None, rot_update=None, torsion_updates=None):
        ...
        if self.time_independent:
            set_time(data, 0, 0, 0, 0, 1, self.all_atom, self.asyncronous_noise_schedule, device=None, include_miscellaneous_atoms=self.include_miscellaneous_atoms)
        else:
            set_time(data, t, t_tr, t_rot, t_tor, 1, self.all_atom, self.asyncronous_noise_schedule, device=None, include_miscellaneous_atoms=self.include_miscellaneous_atoms)
        ...
        if self.time_independent:
            ...
            # atom_rmsd = np.sqrt(((data['atom'].pos.cpu().numpy() - data['atom'].orig_pos.cpu().numpy()) ** 2).sum(axis=1))
            # print('Atom RMSD: ', atom_rmsd)
            # data.atom_y = torch.tensor(atom_rmsd < self.rmsd_cutoff).float()
            data.atom_y = data.y
...
class PDBBind(Dataset):
    def __init__(self, root, transform=None, cache_path='data/cache', split_path='data/', limit_complexes=0, chain_cutoff=10,
                 ...
                 cross_docking=False, use_original_protein_file = False, include_miscellaneous_atoms=False,
                 protein_path_list=None, ligand_descriptions=None, keep_local_structures=False, protein_file="protein_processed",
                 knn_only_graph=False, matching_tries=1):
        ...
        self.full_cache_path = os.path.join(cache_path, f'PDBBind2_limit{self.limit_complexes}'
        ...
        if self.esm_embeddings_path is not None:
            ...
            for key, embedding in id_to_embeddings.items():
                ...
                if key_name in complex_names_all:
                    ...
                    chain_indices_dictlist[key_name].append(int(key.split('_')[2]))
    ...
    def get_complex(self, par):
        ...
        lig = read_mol(self.pdbbind_dir, name, remove_hs=False)
        ...
        try:
            ...
            # TODO chain cutoff
            moad_extract_receptor_structure(path=os.path.join(self.pdbbind_dir, name, f'{name}_protein_processed.pdb'),

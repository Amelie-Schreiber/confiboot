...
from relax.cleanup import read_molecule
from relax import cleanup
import numpy as np
...
def _openmm_minimize(
    ...
    forcefield = openmm_app.ForceField("amber10.xml", "out.xml", "tip3p.xml")
    constraints = openmm_app.HBonds

    # new code to add molecule
    rd_mol = read_molecule('../data/PDBBind_processed/1a28/1a28_ligand.mol2', sanitize=True)
    ...
    #ligand_traj = md.open('../data/PDBBind_processed/1a28/1a28_ligand.')
    ...
    #system = force_field.createSystem(model.topology, constraints=constraints)
    system = force_field.createSystem(model.topology, nonbondedMethod=openmm_app.PME, nonbondedCutoff=1.0 * unit.nanometers,
...
def clean_protein(complex_file_name):
    ...
    alterations_info = {}
    fixed_pdb = cleanup.fix_pdb(complex_file_name, alterations_info)
...
def get_violation_metrics(prot):
...
def run_pipeline(
    ...
    pdb_string = clean_protein(complex_file_name)
...
if __name__ == '__main__':
    run_pipeline('../data/PDBBind_processed/1a28/1a28_protein_processed.pdb')
...
def torsional_loss_function(tr_pred, rot_pred, tor_pred, sidechain_pred, data, t_to_sigma, device, apply_mean=True):
    ...
    if apply_mean:
        tor_loss, tor_base_loss = tor_loss.mean() * torch.ones(1,dtype=torch.float), tor_base_loss.mean() * \
                                  torch.ones(1, dtype=torch.float)
    else:
        index = data['ligand'].batch[data['ligand', 'ligand'].edge_index[0][data['ligand'].edge_mask]].cpu()
    ...
    z = torch.zeros(tor_loss.shape)
    return tor_loss, z, z, tor_loss.detach(), z, z, z, tor_base_loss, z
...
def train_epoch(model, loader, optimizer, device, t_to_sigma, loss_fn, ema_weights, torsional=False):
    model.train()
    ...
    for data in tqdm(loader, total=len(loader)):
        if device.type == 'cuda' and len(data) == 1 or device.type == 'cpu' and data.num_graphs == 1:
            ...
            continue
        if torsional: data = data.to(device)
        optimizer.zero_grad()
        data = [d.to(device) for d in data] if device.type == 'cuda' else data
        try:
            tr_pred, rot_pred, tor_pred, sidechain_pred = model(data) if not torsional else model.torsional_forward(data)
...
def test_epoch(model, loader, device, t_to_sigma, loss_fn, test_sigma_intervals=False, torsional=False):
    model.eval()
    meter = AverageMeter(['loss', 'tr_loss', 'rot_loss', 'tor_loss', 'backbone_loss', 'sidechain_loss',
                          'tr_base_loss', 'rot_base_loss', 'tor_base_loss', 'backbone_base_loss', 'sidechain_base_loss'],

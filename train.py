...
from utils.training import train_epoch, test_epoch, loss_function, inference_epoch_fix, torsional_loss_function
...
def train(args, model, optimizer, scheduler, ema_weights, train_loader, val_loader, t_to_sigma, run_dir, val_dataset2):
    ...
    torsional = args.dataset == 'torsional'
    if torsional:
        loss_fn = torsional_loss_function
    else:
    ...
    for epoch in range(args.n_epochs):
        ...
        logs = {}
        train_losses = train_epoch(model, train_loader, optimizer, device, t_to_sigma, loss_fn, ema_weights if epoch > freeze_params else None, torsional=torsional)
        ...
        val_losses = test_epoch(model, val_loader, device, t_to_sigma, loss_fn, args.test_sigma_intervals, torsional=torsional)
        ...
        if not torsional and args.val_inference_freq != None and (epoch + 1) % args.val_inference_freq == 0:
        ...
        if args.double_val and not torsional and args.val_inference_freq != None and (epoch + 1) % args.val_inference_freq == 0:
            ...
            print("Epoch {}: Val inference on second validation rmsds_lt2 {:.3f} rmsds_lt5 {:.3f} min_rmsds_lt2 {:.3f} min_rmsds_lt5 {:.3f}"
                  .format(epoch, inf_metrics['rmsds_lt2'], inf_metrics['rmsds_lt5'], inf_metrics['min_rmsds_lt2'], inf_metrics['min_rmsds_lt5']))
            logs.update({'valinf2_' + k: v for k, v in inf_metrics.items()}, step=epoch + 1)

        if not torsional and args.train_inference_freq != None and (epoch + 1) % args.train_inference_freq == 0:
        ...
        if epoch > freeze_params:
            ...
            ema_state_dict = copy.deepcopy(model.module.state_dict() if device.type == 'cuda' and not torsional else model.state_dict())
        ...
        state_dict = model.module.state_dict() if device.type == 'cuda' and not torsional else model.state_dict()
...
def main_function():
    ...
    if args.dataset == 'torsional':
        t_to_sigma = partial(t_to_sigma_individual, schedule_type='exponential', sigma_min=args.tor_sigma_min, sigma_max=args.tor_sigma_max)
    else:
